...
import { DEFAULT_FONT_SIZE, DEFAULT_ALIGNMENT, DEFAULT_VERTICAL_ALIGNMENT, DEFAULT_LINE_HEIGHT, DEFAULT_CHARACTER_SPACING, DEFAULT_FONT_COLOR, } from '../text/constants.js';
import { getExtraFormatterSchema, Formatter } from '../text/extraFormatter';
const getLocale = (lang) => ({ en: undefined, zh: zhCN, ja, ko, ar, th, pl, it, de, es, fr }[lang]);
export const getPlugin = ({ type, defaultFormat, icon, inputType, formatsByLang, }) => {
    const plugin = {
        ui: async (arg) => {
            ...
            const { topAdj, bottomAdj } = getBrowserVerticalFontAdjustments(fontKitFont, schema.fontSize ?? DEFAULT_FONT_SIZE, DEFAULT_LINE_HEIGHT, schema.verticalAlignment ?? DEFAULT_VERTICAL_ALIGNMENT);
            ...
            textElement.textContent = value
                ? format(type === 'time' ? new Date(`2021-01-01T${value}`) : new Date(value), schema.format, { locale: getLocale(options?.lang || 'en') })
                : '';
        ...
        propPanel: {
            schema: ({ options, i18n }) => {
                ...
                formatter.buttons = formatter.buttons.filter((button) => button.key !== Formatter.STRIKETHROUGH && button.key !== Formatter.UNDERLINE);
