...
export const validateBarcodeInput = (type, input) => {
    if (!input)
        return false;
 ...
 */
export const barCodeType2Bcid = (type) => type === 'nw7' ? 'rationalizedCodabar' : type;
/**
 *  Strip hash from the beginning of HTML hex color codes for the bwip.js lib
 */
export const mapHexColorForBwipJsLib = (color, fallback) => color ? color.replace('#', '') : fallback ? fallback.replace('#', '') : '000000';
export const createBarCode = async (arg) => {
    ...
    if (barColor)
        bwipjsArg.barcolor = mapHexColorForBwipJsLib(barColor);
    if (textColor)
        bwipjsArg.textcolor = mapHexColorForBwipJsLib(textColor);
    let res;
