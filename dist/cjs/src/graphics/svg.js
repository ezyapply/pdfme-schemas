...
const isValidSVG = (svgString) => fast_xml_parser_1.XMLValidator.validate(svgString) === true;
...
const svgSchema = {
    ui: (arg) => {
        ...
        const container = document.createElement((0, utils_js_1.isEditable)(mode, schema) ? 'textarea' : 'div');
        ...
        if ((0, utils_js_1.isEditable)(mode, schema)) {
            ...
            textarea.style.backgroundColor = (0, utils_js_1.addAlphaToHex)(theme.colorPrimaryBg, 30);
            if (isValidSVG(value)) {
                ...
                const errorElm = (0, utils_js_1.createErrorElm)();
            ...
            if (!value)
                return;
            if (!isValidSVG(value)) {
                rootElement.appendChild((0, utils_js_1.createErrorElm)());
                return;
            }
    ...
    pdf: async (arg) => {
        ...
        const { width, height, position } = (0, utils_js_1.convertForPdfLayoutProps)({ schema, pageHeight });
        const { x, y } = position;
